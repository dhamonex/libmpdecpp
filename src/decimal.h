...
namespace mpdecpp
{
  ...
  public:
     ...
     * @param roundMode the used round mode, if default is used the round mode from decimalInit is
     * used.
     * @return the string representation of the Decimal value.
     */
    std::string toString( unsigned int precision, RoundMode roundMode = RoundMode::Default ) const;
    ...
    friend Decimal floor( const Decimal &value );
    friend Decimal ceil( const Decimal &value );
