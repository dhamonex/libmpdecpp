...
#define CHECK_MPD_SET_STATUS                                                                       \
  CHECK_DECIMAL_OPERATION( "Could not set decimal from value: " +                                  \
...
namespace detail
{
  ...
  {
    if ( precision == 0 ) {
      BOOST_THROW_EXCEPTION( NotInitialized() << ErrorString(
                               "Decimal not initialized. Use Decimal::decimalInit to set precition "
                               "and initialize decimal library" ) );
    }
    ...
    : mpdDecimal{ createDecimal() }
  {
    ...
    if ( !mpd_qcopy( mpdDecimal.get(), other.mpdDecimal.get(), &status ) ) {
      BOOST_THROW_EXCEPTION( DecimalException()
                             << ErrorString( "Could not copy decimal value" ) << ErrorCode( status )
  ...
  ComparisonResult DecimalPrivate::compareToOtherValue( const DecimalPrivate &other )
  {
    ...
    mpd_status_t status{ 0 };
    const auto cmpResult = mpd_qcompare_signal(
      result.get(), mpdDecimal.get(), other.mpdDecimal.get(), threadLocalContext(), &status );
  ...
  {
    applyMpdecimalOperation( &mpd_qmul, ErrorCheckMode::Default, "Multiply failed ({} * {})",
                             other );
  }
  ...
  {
    applyMpdecimalOperation( &mpd_qadd, ErrorCheckMode::Default, "Addition failed ({} + {})",
                             other );
  }

  void DecimalPrivate::subtractAssing( const DecimalPrivate &other )
  {
    applyMpdecimalOperation( &mpd_qsub, ErrorCheckMode::Default, "Subtraction failed ({} - {})",
                             other );
  }

  void DecimalPrivate::divideModAssign( const DecimalPrivate &other )
  {
    applyMpdecimalOperation( &mpd_qrem, ErrorCheckMode::Default, "Remainder failed ({} qrem {})",
                             other );
  }

  void DecimalPrivate::remainderNearAssign( const DecimalPrivate &other )
  {
    applyMpdecimalOperation( &mpd_qrem_near, ErrorCheckMode::Default,
                             "Remainder-Near failed ({} remnear {})", other );
  }
  ...
  {
    applyMpdecimalOperation( &mpd_qlog10, ErrorCheckMode::IgnoreInexactRounding,
                             "Log of base 10 failed ({})" );
  }

  void DecimalPrivate::powAndAssign( const DecimalPrivate &exp )
  {
    applyMpdecimalOperation( &mpd_qpow, ErrorCheckMode::Default, "pow operation failed ({} ** {})",
                             exp );
  }

  void DecimalPrivate::sqrtAndAssign()
  {
    applyMpdecimalOperation( &mpd_qsqrt, ErrorCheckMode::IgnoreInexactRounding,
                             "sqrt operation failed ({})" );
  }
  ...
  {
    applyMpdecimalOperation( &mpd_qminus, ErrorCheckMode::Default,
                             "minus operation on {} failed." );
  }
  ...
  {
    applyMpdecimalOperation( &mpd_qnext_minus, ErrorCheckMode::Default,
                             "Next minus operation on {} failed." );
  }

  void DecimalPrivate::nextPlusAssing()
  {
    applyMpdecimalOperation( &mpd_qnext_plus, ErrorCheckMode::Default,
                             "Next plus operation on {} failed." );
  }

  void DecimalPrivate::nextTowardAssign( const DecimalPrivate &other )
  {
    applyMpdecimalOperation( &mpd_qnext_toward, ErrorCheckMode::Default,
                             "Next toward from {} towards {} failed.", other );
  }

  void DecimalPrivate::quantizeAssign( const DecimalPrivate &other )
  {
    applyMpdecimalOperation( &mpd_qquantize, ErrorCheckMode::IgnoreInexactRounding,
                             "Quantize operation of value {} with exp {} failed.", other );
  }

  void DecimalPrivate::floorAssign()
  {
    applyMpdecimalOperation( &mpd_qfloor, ErrorCheckMode::Default,
                             "Floor operation on {} failed." );
  }
  ...
  std::string DecimalPrivate::toString( const std::string &format, RoundMode roundMode ) const
  {
    ...
    MPDecimalCharPointer result(
      mpd_qformat( mpdDecimal.get(), format.c_str(), roundModeGuard.guardContext, &status ) );

    if ( status != 0 ) {
      BOOST_THROW_EXCEPTION(
        DecimalException() << ErrorString( "Could not format decimal value with format " + format )
